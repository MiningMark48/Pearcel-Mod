...
public class ItemSAP extends ItemPearcelMod{
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        par3List.add(StatCollector.translateToLocal("tooltip.item.sap.line6"));
    }
    ...
    public ItemStack onItemRightClick(ItemStack item, World world, EntityPlayer player){
        ...
        if (player.experienceLevel >= xpNeeded) {
            if (!player.isSneaking() && player.getHeldItem().getItem() == ModItems.sap) {
                if (player.getHeldItem().getItem() != null && player.getHeldItem().getItem() == ModItems.sap) {
                    if (!world.isRemote) {
                        ...
                        //Horse
                            ...
                            if (!world.isRemote) {
                                ...
                                world.spawnEntityInWorld(new EntityLightningBolt(world, player.posX, player.posY, player.posZ));
                                player.experienceLevel = player.experienceLevel - xpNeeded + (xpNeeded/2);
                            }
                        }
                    }
                    ...
                    player.getHeldItem().damageItem(1, player);
                    player.experienceLevel = player.experienceLevel - xpNeeded;
                } else {
                    if (!world.isRemote) {
            ...
            if (!world.isRemote){
                player.addChatComponentMessage(new ChatComponentTranslation(EnumChatFormatting.DARK_RED + StatCollector.translateToLocal("chat.sap.nexp") + EnumChatFormatting.AQUA + " " + (xpNeeded - player.experienceLevel)));
            }
        }
