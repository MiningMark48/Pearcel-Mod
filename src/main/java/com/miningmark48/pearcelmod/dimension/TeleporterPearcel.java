...
public class TeleporterPearcel extends Teleporter{
    ...
    public TeleporterPearcel(WorldServer worldserver){
        super(worldserver);
            ...
            for (int l = -2; l <= 2; ++l){
                for (int i1 = -2; i1 <= 2; ++i1){
                    for (int j1 = -1; j1 < 3; ++j1){
                        ...
                        int i2 = k + i1 * b1 - 1 * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlock(k1, l1, i2, flag ? ModBlocks.pearcelBlock : Blocks.air);
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8){
        ...
        if (this.destinationCoordinateCache.containsItem(j1)){
            TeleporterPearcel.PortalPosition portalpostion = (TeleporterPearcel.PortalPosition)this.destinationCoordinateCache.getValueByKey(j1);
            d3 = 0.0D;
            i = portalpostion.posX;
            j = portalpostion.posY;
            k = portalpostion.posZ;
            portalpostion.lastUpdateTime = this.worldServerInstance.getTotalWorldTime();
            flag = false;
        }else{
            for (l3 = l - short1; l3 <= 1 + short1; ++l3){
                double d4 = (double)l3 + 0.5D - entity.posX;
                for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1){
                    double d5 = (double)l1 + 0.5D - entity.posZ;
                    for (int i2 = this.worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2){
                        if (this.worldServerInstance.getBlock(l3, i2, l1) == ModBlocks.pearcelPortal){
                            while (this.worldServerInstance.getBlock(l3, i2 - 1, l1) == ModBlocks.pearcelPortal){
                                --i2;
                            }
        ...
        if (d3 <= 0.0D){
            ...
            if (this.worldServerInstance.getBlock(i - 1, j, k) == ModBlocks.pearcelPortal){
                i4 = 2;
            }
            if (this.worldServerInstance.getBlock(i + 1, j, k) == ModBlocks.pearcelPortal){
                i4 = 0;
            }
            if (this.worldServerInstance.getBlock(i, j, k - 1) == ModBlocks.pearcelPortal){
                i4 = 3;
            }
            if (this.worldServerInstance.getBlock(i, j, k + 1) == ModBlocks.pearcelPortal){
                i4 = 1;
        ...
        for (i3 = 0; i3 < 4; ++i3)
        {
            for (j3 = 0; j3 < 4; ++j3)
            {
                for (k3 = -1; k3 < 4; ++k3)
                {
                    ...
                    this.worldServerInstance.setBlock(l3, i4, j4, (Block)(flag ? Blocks.stone : ModBlocks.pearcelPortal), 0, 2);
                }
            }
    ...
    public void removeStalePortalLocations(long p_85189_1_)
    {
        if (p_85189_1_ % 100L == 0L)
        {
            ...
            while (iterator.hasNext())
            {
                ...
                TeleporterPearcel.PortalPosition portalposition = (TeleporterPearcel.PortalPosition)this.destinationCoordinateCache.getValueByKey(olong.longValue());
